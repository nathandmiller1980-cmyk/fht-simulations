...
export function sumZA(list: {Z:number; A:number; count?:number}[]) {
  let Z=0, A=0;
  for (const x of list) { const c = x.count ?? 1; Z += x.Z*c; A += x.A*c; }
  return {Z, A};
}

export function qValueMeV(inputs: {massU?:number; count?:number}[], outputs: {massU?:number; count?:number}[]) {
...
export function conservationVerdict(inpZA:{Z:number;A:number;count?:number}[], outZA:{Z:number;A:number;count?:number}[]) {
  const i = sumZA(inpZA), o = sumZA(outZA);
  if (i.Z !== o.Z || i.A !== o.A) return {ok:false, reason:`Conservation violated: in(Z=${i.Z},A=${i.A}) != out(Z=${o.Z},A=${o.A})`};
  return {ok:true};
}
